...
## GPL release requirements

The first GPL versions are extension 0.4.0, engine 0.2.0, and CLI 0.2.0. Announce the MIT to GPL-3.0-or-later transition prominently in release notes. Preserve earlier MIT tags and artifacts.

Before packaging, run `npm ci`, regenerate notices with `node scripts/third-party-notices.js`, review any dependency/license changes, and stage all intended source files in Git. Packaging rejects untracked, non-ignored files; ignore only unrelated files. Packaging captures current tracked source, installed production dependencies, and checksum-verified upstream source snapshots (cached under the gitignored `upstream-sources/` directory after the first download). When a compiled dependency changes, update and verify its entry in `scripts/upstream-sources.json`; review new dependencies for missing preferred source too.

The test suite requires Git for its packaging regression fixtures. Build and test with `npm run compile`, `npm test`, and `npm run build`. Create artifacts with `npm run package`, `npm pack -w @talktomebaby/engine --pack-destination .artifacts`, and `npm pack -w talktomebaby-cli --pack-destination .artifacts` (create `.artifacts` first). Run `npm run test:packaging` (Python 3.9+ required) to check their licenses, notices, and embedded source. Extract one source archive into a fresh directory and verify `npm ci`, `npm run build`, and `npm test` there. See [LICENSING.md](LICENSING.md) for recipient rebuild instructions.

Publish the validated artifacts without rebuilding from a different checkout. Keep `source.tar.gz`, `LICENSE`, `LICENSING.md`, and `THIRD_PARTY_NOTICES.md` in every distributed package. Attach a copy of the matching source archive next to the binary assets on GitHub Releases and retain it for as long as the binary is offered. Ensure Marketplace/npm documentation explains where to find the included source.
