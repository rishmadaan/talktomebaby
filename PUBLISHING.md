...
Before packaging, run `npm ci`, regenerate notices with `node scripts/third-party-notices.js`, review any dependency/license changes, and stage all intended source files in Git. Packaging captures current tracked source, installed production dependencies, and checksum-verified upstream source snapshots. When a compiled dependency changes, update and verify its entry in `scripts/upstream-sources.json`; review new dependencies for missing preferred source too.

Build and test with `npm run compile`, `npm test`, and `npm run build`. Create artifacts with `npm run package`, `npm pack -w @talktomebaby/engine --pack-destination .artifacts`, and `npm pack -w talktomebaby-cli --pack-destination .artifacts` (create `.artifacts` first). Run `npm run test:packaging` (Python 3.9+ required) to check their licenses, notices, and embedded source. Extract one source archive into a fresh directory and verify `npm ci`, `npm run build`, and `npm test` there. See [LICENSING.md](LICENSING.md) for recipient rebuild instructions.
