...
**Extension ↔ Webview messages:** `playAudio` (base64 data URL + playback metadata), `pause`, `resume`, `stop`, `audioEnded` (with playback metadata), `togglePauseResume`, `stopPlayback`, `ready`.
...
- **cache.ts** — LRU in-memory audio cache, 100MB cap, keyed by SHA256 of (text + provider + voice + speed).
